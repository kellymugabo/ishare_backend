...
class UserSerializer(serializers.ModelSerializer):
    is_verified = serializers.SerializerMethodField() # ✅ Add verification status
...
# -------------------- REVIEWS (NEW HELPER) --------------------
# ✅ Used inside UserProfile to format the reviews list
...
class UserProfileSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'phone_number', 
            'profile_picture', 'bio',
            'rating', 'reviews', # ✅ Added rating & reviews
    ...
    # ✅ Logic to calculate dynamic average
...
class TripSerializer(serializers.ModelSerializer):
    ...
    driver_rating = serializers.SerializerMethodField() # ✅ Added rating for cards
    ...
    class Meta:
        model = Trip
        fields = [
            'id', 'driver', 'driver_name', 'driver_phone', 'driver_rating', # ✅ Added
    ...
    def get_booked_seats(self, obj):
        try:
            booked = obj.bookings.filter(status='ACCEPTED').aggregate(
                total=Sum('seats_booked')
            )['total']
        except AttributeError:
            booked = obj.booking_set.filter(status='ACCEPTED').aggregate(
                total=Sum('seats_booked')
            )['total']
        return booked or 0

    def get_driver_rating(self, obj):
        # ✅ Calculate average rating for the driver on the trip card
...
class RatingSerializer(serializers.ModelSerializer):
    ...
    trip = serializers.PrimaryKeyRelatedField(read_only=True) # Read-only for output
    trip_id = serializers.PrimaryKeyRelatedField(
        queryset=Trip.objects.all(),
        source='trip',
        write_only=True
    ...
    class Meta:
        model = Rating
        # ✅ Ensure these fields match what Flutter sends
